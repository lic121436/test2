...
if($act === reg){
    // 完成注册功能
    $regtime = time(); // 注册时间
    $token = md5($username.$password.$regtime); // 令牌
    $token_exptime = $regtime + 24*3600; // 过期时间
    $data = compact('username', 'password', 'email', 'token', 'token_exptime', 'regtime');
    $res = $PdoMySQL->add($data, $table); // 调用封装好的添加函数 add();
    $lastInsertId = $PdoMySQL->getLastInsertId(); // 获取插入的ID
    if($res){
        // 发送邮件， 以QQ邮箱为例
        // 配置邮件服务器， 得到传输对像
        $transport = Swift_SmtpTransport::newInstance('smtp.163.com', 465, 'ssl');
        // 设置登陆账号和密码
        $transport -> setUsername('lic121436@163.com');
        $transport -> setPassword('email163lic');
        // 得到发送邮件对像Swift_Mailer对象
        $mailer = Swift_Mailer::newInstance($transport);
        // 得到邮件信息对象
        $message = Swift_Message::newInstance();
        // 设置管理员的信息
        $message->setFrom(array('21528489@qq.com' => '管理员'));
        ...
        <a href="{$url}">{$urlencode}</a><br/>
                    如果点此连接无反映，可以将其复制到浏览器中来执行，链接的有效时间为24小时。
EOF;
